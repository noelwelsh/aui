...
extension [Alg <: Algebra, A](component: Component[Alg, A]) {
  ...
  def and[Alg2 <: Algebra, B](that: Component[Alg2, B]): Component[Alg & Alg2 & Layout, (A, B)] =
...
object Component {
  ...
  def textField(label: String, placeholder: String = "", validation: Validation[String] = Validation.succeed): Component[Controls, String] =
    alg => alg.textField(label, placeholder, validation)

  def choice[A](label: String, options: Seq[(String, A)]): Component[Controls, A] =
